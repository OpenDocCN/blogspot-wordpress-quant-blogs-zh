...
# Neo4j Node Data Storage | Tales from a Trading Desk

> 来源：[https://mdavey.wordpress.com/2016/03/14/neo4j-node-data-storage/#0001-01-01](https://mdavey.wordpress.com/2016/03/14/neo4j-node-data-storage/#0001-01-01)

## Neo4j Node Data Storage

[Neo4j](http://neo4j.com/) (graph database) is cool piece of technology.  Graph databases,  need to be used appropriately, they like blockchain, and other technologies in the sweet store, are not silver bullets in their own right.

When considering what to store in a graph database, its easy to leverage the nodes and linkage between nodes, relationships, to generate a graph, and traverse a graph.  What is unclear, or not well documented, is how much data you should store on nodes. For example,  Christophe Willemsen [posting](http://graphaware.com/neo4j/2015/01/16/neo4j-graph-model-design-labels-versus-indexed-properties.html) on GraphAware with regards to using Neo4j to model blogs with postings that are followed by users has clear graph benefits.  What isn’t clear is if the postings themselves which could be lengthy in terms of text, and may include images at a minimum, should also be stored on the post node as a [property/label](http://stackoverflow.com/questions/31028504/what-is-the-difference-between-a-label-and-a-property-in-neo4j)?

GitHub [neo4j-contrib/neo4j-faq](https://github.com/neo4j-contrib/neo4j-faq) offer the following, which maybe relevant:
...
The 1st edition (unfortunately I don’t have the latest version) of [Graph Databases](http://graphdatabases.com/) by Ian Robinson, Jim Webber & Emil Eifrem provides the following:
